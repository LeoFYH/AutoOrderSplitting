...
def create_app() -> Flask:
    ...
    def process_orders():
        ...
        ignored_template_warning = ""
        if template_file is not None and template_file.filename:
            ignored_template_warning = f"生成采购单时已忽略请求中的模板文件「{template_file.filename}」，使用当前保存的采购总模板。"

        template_items = load_template_items()
        if not template_items:
            return jsonify({"error": "请先上传采购总模板"}), 400
        if not CURRENT_TEMPLATE.exists():
            return jsonify({"error": "请重新上传采购总模板"}), 400
        ...
        if ignored_template_warning:
            result.warnings.insert(0, ignored_template_warning)
